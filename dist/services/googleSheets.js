...
function parsePaymentMeta(raw) {
    ...
    if (Number.isFinite(msgId))
        out.paymentMessageId = msgId;
    if (paidEventId)
        out.paidEventId = paidEventId;
    return out;
}
async function updateAppointmentPaymentMeta(appointmentId, meta) {
    if (!GOOGLE_SHEET_ID)
        return;
    ...
    const rowIndex = rows.findIndex((row) => row[11] === appointmentId);
    if (rowIndex === -1)
        return;
...
function parseSlotCapacities(raw) {
    const capacities = { default: 3 };
    if (!raw)
        return capacities;
...
export async function getDoctors() {
    ...
    try {
        ...
        return dataRows.map((row) => ({
            ...
            doctorName: row[0] || '',
            specialty: row[1] || '',
            slots: row[2] ? row[2].split(',').map((s) => s.trim()) : [],
...
export async function addAppointment(appointment) {
    ...
    try {
        ...
        const rows = response.data.values || [];
        let nextRowIndex = rows.findIndex((row, index) => index > 0 && (!row || row.length === 0 || !row[0]));
        if (nextRowIndex === -1) {
...
export async function updateAppointmentDetails(appointment) {
    if (!GOOGLE_SHEET_ID)
        return;
    try {
        const response = await sheets.spreadsheets.values.get({
        ...
        const rows = response.data.values || [];
        const rowIndex = rows.findIndex((row) => row[11] === appointment.appointmentId); // Column L (ID)
        if (rowIndex !== -1) {
            const actualRowIndex = rowIndex + 2;
            await sheets.spreadsheets.values.update({
                spreadsheetId: GOOGLE_SHEET_ID,
...
export async function getAppointmentsToday() {
    ...
    try {
        ...
        return rows
            .filter((row) => row[5] === today) // Column F is Date (index 5)
            .map((row) => ({
            patientName: row[0],
            age: row[1],
            gender: row[2],
...
export async function getAppointmentById(appointmentId) {
    ...
    try {
        ...
        const rows = response.data.values || [];
        const row = rows.find((r) => r[11] === appointmentId); // Column L (ID)
        if (row) {
...
export async function markReminderSent(appointmentId) {
    ...
    try {
        ...
        const rows = response.data.values || [];
        const rowIndex = rows.findIndex((row) => row[11] === appointmentId); // Column L (ID)
        if (rowIndex !== -1) {
            const actualRowIndex = rowIndex + 2;
            await sheets.spreadsheets.values.update({
                spreadsheetId: GOOGLE_SHEET_ID,
...
export async function getSlotCounts(doctorName, date) {
    if (!GOOGLE_SHEET_ID)
        return {};
    try {
        const response = await sheets.spreadsheets.values.get({
...
export async function getAppointmentsByChatId(chatId) {
    if (!GOOGLE_SHEET_ID)
        return [];
    try {
        const response = await sheets.spreadsheets.values.get({
        ...
        const rows = response.data.values || [];
        return rows
            .filter((row) => row[12] === chatId && row[8] !== 'Failed') // Column M is Chat ID, I is Status
            .map((row) => ({
            patientName: row[0],
            age: row[1],
            gender: row[2],
...
export async function updateAppointmentReport(appointmentId, reportLink) {
    const apt = await getAppointmentById(appointmentId);
    if (apt) {
...
export async function updateAppointmentPaymentMessageId(appointmentId, messageId) {
    await updateAppointmentPaymentMeta(appointmentId, { paymentMessageId: messageId });
}
export async function updateAppointmentPaidEventId(appointmentId, paidEventId) {
