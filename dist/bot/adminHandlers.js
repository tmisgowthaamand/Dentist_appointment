...
const sendAdminMenu = async (ctx) => {
    ...
    await ctx.replyWithPhoto('https://images.unsplash.com/photo-1551076805-e1869033e561?auto=format&fit=crop&q=80&w=1000', {
        caption: welcomeMsg,
        parse_mode: 'Markdown',
...
export function setupAdminBotHandlers(bot) {
    ...
    bot.on('text', async (ctx, next) => {
        ...
        if (isAdmin(ctx) && !authenticatedAdmins.has(userId)) {
            ...
            if (text === password) {
                ...
                return bot.handleContext(newCtx);
            }
            else {
    ...
    bot.hears('📊 Today\'s Schedule', (ctx) => {
        if (isAdmin(ctx) && authenticatedAdmins.has(ctx.from.id.toString())) {
            return bot.handleContext(Object.assign(Object.create(ctx), { message: { text: '/today' } }));
        }
    });
    bot.hears('📈 Statistics', (ctx) => {
        if (isAdmin(ctx) && authenticatedAdmins.has(ctx.from.id.toString())) {
            return bot.handleContext(Object.assign(Object.create(ctx), { message: { text: '/stats' } }));
        }
    });
    bot.hears('📖 History Help', (ctx) => {
        if (isAdmin(ctx) && authenticatedAdmins.has(ctx.from.id.toString())) {
            return bot.handleContext(Object.assign(Object.create(ctx), { message: { text: '/history' } }));
        }
    });
    ...
    const checkAuth = async (ctx) => {
        const userId = ctx.from?.id.toString() || '';
        if (!isAdmin(ctx))
            return false; // Ignore random strangers
        if (!authenticatedAdmins.has(userId)) {
