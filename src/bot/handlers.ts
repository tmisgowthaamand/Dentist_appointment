...
import type { UserState, Appointment } from '../types/index.js';
...
export function setupBotHandlers(bot: Telegraf<Context>, adminBot: Telegraf<Context>) {
    ...
    bot.on(['document', 'photo'], async (ctx) => {
        ...
        try {
            ...
            if (process.env.ADMIN_CHAT_ID) {
                await adminBot.telegram.sendMessage(process.env.ADMIN_CHAT_ID,
                    `📄 *New Medical Report Uploaded*\n` +
                    `━━━━━━━━━━━━━━━\n` +
                    `👤 *Patient:* ${state.patientName}\n` +
                    `🔗 *Appointment ID:* \`${appointmentId}\`\n\n` +
                    `📥 *View:* [Open Report](${publicLink})\n\n` +
                    `💡 *Shortcut:* \`/history ${appointmentId}\``,
                    { parse_mode: 'Markdown' }
                ).catch(e => console.error('Failed to notify admin of report upload:', e));
            }
    ...
    async function handleTextMessage(ctx: Context, text: string) {
        const chatId = ctx.from!.id;
        const state = userStates[chatId];
    ...
    async function proceedToDoctorSelection(ctx: Context, state: UserState) {
    ...
    bot.on('callback_query', async (ctx) => {
        ...
        if (data.startsWith('GENDER_')) {
            ...
            const mode = data.replace('PAY_', '') as 'Online' | 'Clinic';
...
📌 *Note:* Please pay ₹100 at the front desk when you arrive. See you soon! 🦷`;
    ...
    bot.action(/CONFIRM_CANCEL_(.+)/, async (ctx) => {
        ...
        try {
            ...
            if (!appointment) return ctx.reply('❌ Appointment not found.');
...
export function getStateForUser(chatId: number): UserState | undefined {
    return userStates[chatId];
}

export function setUserState(chatId: number, state: Partial<UserState>) {
