...
async function callWithRetry(fn: () => Promise<any>, retries = 3, delay = 5000): Promise<any> {
    try {
        return await fn();
    } catch (error: any) {
...
export async function generateReply(text: string, language: string, context: string): Promise<string> {
